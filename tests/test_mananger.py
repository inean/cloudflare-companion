...
from dns_synchub.mappers import BaseMapper
from dns_synchub.pollers import BasePoller
...
@pytest.fixture
def mock_poller_infinity(mock_poller: MagicMock):
    async def run_indenfinitely(timeout: float):
        while True:
...
@pytest.fixture
def mock_mapper():
    return MagicMock(spec=BaseMapper)


@pytest.fixture
def data_manager(mock_logger: MagicMock):
    class MockList(list[asyncio.Task[None]]):
        def clear(self): ...

    manager = DataManager(logger=mock_logger)
    manager.tasks = MockList()
    manager.tasks.clear = MagicMock(spec=list[asyncio.Task[None]].clear, return_value=None)
    return manager


def test_initialization(data_manager: DataManager, mock_logger: Logger):
...
def test_add_poller(data_manager: DataManager, mock_poller: AsyncMock):
    data_manager.add_poller(mock_poller, backoff=5.0)
    assert (mock_poller, 5.0) in data_manager.pollers


@pytest.mark.asyncio
async def test_add_mapper(data_manager: DataManager, mock_mapper: MagicMock):
...
@pytest.mark.asyncio
async def test_start(data_manager: DataManager, mock_poller: AsyncMock, mock_mapper: MagicMock):
    ...
    cast(MagicMock, data_manager.tasks.clear).assert_called_once()


@pytest.mark.asyncio
async def test_start_no_mapper(data_manager: DataManager, mock_poller: AsyncMock):
    data_manager.add_poller(mock_poller, backoff=5.0)
    await data_manager.start(timeout=10.0)
    mock_poller.run.assert_awaited_once_with(timeout=10.0)
...
@pytest.mark.asyncio
async def test_start_no_pollers(data_manager: DataManager, mock_mapper: MagicMock):
    mock_event_emitter = MagicMock(spec=EventEmitter)
    mock_event_emitter.__len__.return_value = 1
    data_manager.mappers = mock_event_emitter
...
@pytest.mark.asyncio
async def test_start_cancel_interrupt(data_manager: DataManager, mock_poller_infinity: AsyncMock):
...
@pytest.mark.asyncio
async def test_stop(data_manager: DataManager, mock_poller_infinity: AsyncMock):
...
def test_aggregate_data(data_manager: DataManager, mock_poller: AsyncMock):
    data_manager.add_poller(mock_poller, backoff=5.0)
